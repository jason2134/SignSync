...
def prob_viz(res, actions, input_frame, colors):
    ...
    left_column_letters = 20  # Adjusted number of letters on the left side
...
model = tf.keras.models.load_model('Model (CNN&MediaPipe)/best_model.h5') 
...
with mp_hands.Hands(
    ...
    while cap.isOpened():
        ...
        if len(sequence) == 30:
            window = np.array(sequence)                  # (30, 126)
            window_flat = window.flatten()[None, :]      # → (1, 30*126)
            res = model.predict(window_flat)[0]          # ANN → softmax vector
