...
const SignLanguageButton: React.FC<ButtonProps> = ({ t, 'aria-label': ariaLabel, className, isListenOnly }) => {
    ...
    handsRef.current?.onResults(async (results: HandResults) => {
      try {
        ...
        if (isStable && prediction !== 'Unknown') {
          ...
          APP.conference._room.sendCommand('sign_language', { value: '' });
          updateSubtitles('')
        }
        tensor.dispose();
      } catch (error) {
...
const SignLanguageOverlay: React.FC<OverlayProps> = ({ subtitles, error, t, isListenOnly }) => {
  ...
  useEffect(() => {
      ...
      if (subtitles.trim()) {
        setPredictions(prev => {
          ...
          // Limit predictions to prevent excessive growth
          return newPredictions.slice(-10);
        });
      }
    }, [subtitles, error, t]);
