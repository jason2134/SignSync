...
const SignLanguageOverlay: React.FC<OverlayProps> = ({ subtitles, error, t }) => {
  ...
  // Handle updates to subtitles and errors
  useEffect(() => {
  ...
  if (!predictions.length) {
    console.log('Overlay not rendering: no predictions');
    return null;
  }

  console.log('Overlay rendering with predictions:', predictions);
    ...
    <>
      ...
      <div className="sign-language-overlay" aria-live="polite">
        <span className="prediction-text">
          {predictions.map((text, index) => `${text}${index < predictions.length - 1 ? ' ' : ''}`).join('')} 
        </span>
...
// ${index < predictions.length - 1 ? ' ' : ''} adds spacing to the predicted signs
