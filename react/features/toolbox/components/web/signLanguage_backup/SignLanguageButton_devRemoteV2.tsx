...
const initialState: SignLanguageState = { text: '', error: null, isListenOnly: false };
...
const signLanguageReducer = (state = initialState, action: { type: string; text?: string; error?: string; isListenOnly?: boolean }) => {
  switch (action.type) {
    ...
    case 'TOGGLE_LISTEN_ONLY':
      ...
      return { ...state, isListenOnly: action.isListenOnly ?? !state.isListenOnly };
    default:
      return state;
  }
};
...
const extractKeypoints = (results: HandResults | null): KeypointFrame => {
  ...
  for (let idx = 0; idx < handCount; idx++) {
    ...
    const keypoints = landmarks
      .map((landmark: any) => [1 - landmark.x, landmark.y, landmark.z]) // Flip x-coordinate
      .flat();
    ...
    console.debug(`Hand ${idx}: Label=${handedness}, Keypoints (first 6)=${keypoints.slice(0, 6)}`);
    if (handedness === 'Right') { // User's left hand in mirrored image
      keypoints.forEach((val, i) => (leftHand[i] = val));
    } else { // User's right hand
      keypoints.forEach((val, i) => (rightHand[i] = val));
    } 
  }//1. Flip the coordinates first, then assign the coords with the opposite label to address MIRRORING

  console.debug('Extracted keypoints (flipped):', {
    leftHand: leftHand.slice(0, 6), // User's left hand
    rightHand: rightHand.slice(0, 6), // User's right hand
  });
  return [...leftHand, ...rightHand] as KeypointFrame;
};
...
const SignLanguageButton: React.FC<ButtonProps> = ({ t, 'aria-label': ariaLabel, className }) => {
    ...
    const initialize = async () => {
      try {
        ...
        APP.conference._room.addCommandListener('sign_language', (data, participantId) => {
            ...
            // Show it in UI, console, etc.
            console.log(`Received sign from ${participantId}: ${detectedSign}`);
            // Check if listen-only mode is enabled
                    const isListenOnly = APP.store.getState()['features/sign-language']?.isListenOnly || false;
                    if (isListenOnly) {
  ...
  const preprocessFrame = async (results: HandResults): Promise<tf.Tensor> => {
    ...
    try {
      return tf.tidy(() => {
        ...
        console.debug('Normalized keypoints (first frame):', {
          leftHand: normalizedSequence[0].slice(0, 6), // User's left hand
          rightHand: normalizedSequence[0].slice(63, 69), // User's right hand
        });
  ...
  const predictSign = async (tensor: tf.Tensor): Promise<string> => {
    ...
    try {
      ...
      prediction.dispose();
      return confidence > 0.85 ? labels[labelIndex] : 'Unknown'; // 0.7
    } catch (error) {
  ...
  const getLocalVideoStream = (): any | null => {
    ...
    if (videoTrack && typeof videoTrack.getTrack === 'function') {
      ...
      console.warn('Video track or getTrack method unavailable:', videoTrack);
    }
  ...
  const processVideoFrames = (videoTrack: any): (() => void) => {
    ...
    const processFrame = async (timestamp: number) => {
      if (!shouldContinue || !videoTrack) {
        console.debug('Stopping frame processing: Invalid track');
        return;
      }
    ...
    videoElement.width = 640; // Set explicit pixel dimensions
    ...
    videoElement.onloadedmetadata = () => {
      console.debug('Video metadata:', {
        width: videoElement.videoWidth,
        height: videoElement.videoHeight,
        readyState: videoElement.readyState,
        duration: videoElement.duration,
      });
      // Adjust videoElement size to match actual video dimensions
    ...
    const camera = new Camera(videoElement, {
      onFrame: async () => {
        if (handsRef.current && videoElement.readyState >= 2 && videoElement.videoWidth > 0 && videoElement.videoHeight > 0) {
          console.debug('Sending frame to MediaPipe:', {
          ...
          console.warn('Skipping frame: video not ready or invalid dimensions', {
            videoWidth: videoElement.videoWidth,
            videoHeight: videoElement.videoHeight,
            readyState: videoElement.readyState,
          });
    ...
    handsRef.current?.onResults(async (results: HandResults) => {
      try {
        ...
        if (isStable && prediction !== 'Unknown') {
          APP.conference._room.sendCommand(
            'sign_language',
            {
              value: prediction
            }
          )
          console.log("Sign broadcasted:", prediction)
  ...
  const waitForConference = () => {
    return new Promise<void>((resolve) => {
      if (APP.conference && APP.conference.isJoined()) {
        ...
        APP.store.dispatch({ type: 'SET_SIGN_LANGUAGE_ERROR', error: 'errors.conferenceNotJoined' });
...
const ListenOnlySignLanguageButton: React.FC<ListenOnlyButtonProps> = ({ t, 'aria-label': ariaLabel, className, isListenOnly, dispatch }) => {
  const handleClick = () => {
    console.log('Listen-only button clicked, current state:', isListenOnly);
    dispatch({ type: 'TOGGLE_LISTEN_ONLY', isListenOnly: !isListenOnly });
    console.log('New listen-only state:', !isListenOnly);
  };
