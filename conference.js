...
function broadcastDataChannelMessage(message) {
    if (!this._room) {
        console.warn('Cannot broadcast message: Conference room not initialized');
        return false;
    }
    if (!this.isJoined()) {
        console.warn('Cannot broadcast message: Conference not joined');
        return false;
    }
    try {
        const messageString = JSON.stringify(message);
        this._room.sendMessage(messageString);
        console.log('Data channel message sent:', message);
        return true;
    } catch (error) {
        console.error('Error broadcasting data channel message:', error);
        return false;
    }
}
...
export default {
    ...
    getLocalVideoStream() {
        ...
        const filmstripState = state['features/filmstrip'];
        const participantsState = state['features/base/participants'];
        
        console.log('Filmstrip state:', filmstripState);
        console.log('Participants state:', participantsState);
    
        // Declare the variable and safely access the local participant's ID
        let localParticipantId;
        if (participantsState.local && participantsState.local.id) {
            localParticipantId = participantsState.local.id;
            console.log('Local participant ID from Redux:', localParticipantId);
        } else {
            console.log('Local participant not found in Redux state');
        }
    
        // Get local ID from conference as a fallback
        ...
        // Convert the video track to JSON and write to file
        //const videoTrackJson = JSON.stringify(videoTrack, null, 2);
        //.writeFileSync('videoTrack.json', videoTrackJson, 'utf8');
        //console.log('Video track data written to videoTrack.json');
        return videoTrack;
    },

    getRmoteSignerVideoStream(signerId){
        const conference = APP.conference._room;
        if (!conference) {
            console.log('No conference room available');
            return null;
        ...
        console.log('Participants state:', participantsState);
        let videoTrack = null;
        // Remote participant case
        const participant = conference.getParticipantById(pinnedParticipantId);
        if (!participant) {
            console.log('No remote participant found for ID:', pinnedParticipantId);
            return null;
        }
        const tracks = participant.getTracks();
        console.log('Tracks from remote participant:', tracks);
        if (!tracks || tracks.length === 0) {
            console.log('No tracks available for remote participant');
            return null;
        }
        videoTrack = tracks.find(track => track.getType() === 'video');
        if (!videoTrack) {
            console.log('No video track found for remote participant');
            return null;
        }
        console.log('Video track retrieved from remote participant:', videoTrack);
        return videoTrack;

    },

    // Define getPinnedVideoStream as a method of the exported objec
    getPinnedVideoStream() {
        const conference = APP.conference._room;
        if (!conference) {
            console.log('No conference room available');
            return null;
        ...
        // Get pinned participant ID from filmstrip
        let pinnedParticipantId = null;
        if (filmstripState && filmstripState.activeParticipants) {
            const pinnedParticipant = filmstripState.activeParticipants.find(
                participant => participant.pinned
            );
            if (pinnedParticipant) {
                pinnedParticipantId = pinnedParticipant.participantId;
                console.log('Pinned participant ID from filmstrip:', pinnedParticipantId);
            }
        }
    
        // Fallback to local participant ID from Redux
        if (!pinnedParticipantId && participantsState.local) {
            pinnedParticipantId = participantsState.local.id;
            console.log('Falling back to local participant ID from Redux:', pinnedParticipantId);
        }
    
        if (!pinnedParticipantId) {
            console.log('No participant ID available');
            return null;
        }
    
        // Get local ID from conference
        const localId = conference.myUserId();
        console.log('Local ID from conference.myUserId():', localId);
    
        let videoTrack = null;
    
        // Check if the pinned ID matches the local user or is local by context
        if (pinnedParticipantId === localId || participantsState.local.id === pinnedParticipantId) {
            // Local participant case
            const localTracks = conference.getLocalTracks();
            console.log('Local tracks:', localTracks);
            if (!localTracks || localTracks.length === 0) {
                console.log('No local tracks available');
    ...
    getPinnedVideoStream() {
        ...
        // Get the pinned participant ID from Redux state
        const state = APP.store.getState();
        console.log('Full Redux state:', state); // Log entire state
        console.log('Participants state:', state['features/base/participants']);

        const pinnedParticipantId = state['features/base/participants']?.pinnedParticipant;

        if (!pinnedParticipantId) {
            console.log('No pinned participant found in Redux state');
            return null;
        }

        // Get the participant object by ID
        const participant = conference.getParticipantById(pinnedParticipantId);
        if (!participant) {
            console.log('Pinned participant not found in conference');
            return null;
        }
        ...
        const tracks = participant.getTracks();
        if (!tracks || tracks.length === 0) {
            console.log('No tracks available for pinned participant');
            return null;
        }
        ...
        if (!videoTrack) {
            console.log('No video track found for pinned participant');
            return null;
        }

        console.log('Pinned video track retrieved:', videoTrack);
        return videoTrack;
    },*/
    ...
    async init({ roomName, shouldDispatchConnect }) {
        ...
        // Expose getPinnedVideoStream to APP.conference (globally accessible via window.APP.conference)
        if (!APP.conference.getPinnedVideoStream) {
            APP.conference.getPinnedVideoStream = this.getPinnedVideoStream.bind(this);
            console.log('getPinnedVideoStream exposed to APP.conference');
        }
    ...
    useVideoStream(newTrack) {
        ...
        return new Promise((resolve, reject) => {
            _replaceLocalVideoTrackQueue.enqueue(onFinish => {
                ...
                // If transcribe enabled --> place newtrack to ML model for realtime transcribe
